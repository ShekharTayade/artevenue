	...
	if not user_ProfileForm:
		if curruser_profile:
			if businessprofile_form:
				...
				if curruser_profile:
					if curruser_profile.business_profile:
						b_code = curruser_profile.business_profile
					else:
						b_code = None
				else:
					b_code = None
...
def my_business_report(request, user_id=None):
	...
	orders = Order.objects.filter( user_id__in = client_ids, order_status = 'PC' )
	
	clients = User.objects.filter(id__in = client_ids).annotate(ord_amt=Sum('order__order_total'),
		ord_num=Count('order__order_id'))
	
	client_total = clients.count()
	clients_with_orders = orders.values('user').distinct().count()
	ord_total = orders.aggregate(ord_amt=Sum('order_total'))
...
def referral_fee_month_process(yrmonth=''):
	...
	if yrmonth == '':
		last_mth_yr = datetime.now() - relativedelta(months=1)
	...
	for b in business_profiles:
		...
		ord_value = Order.objects.filter(order_date__gte = year_start_date,
				...
				user_id = b.user_id).aggregate(sum = Sum('order_total'))
		total_order_value =0
		if ord_value :
			if ord_value['sum']:
		...
		#### Orders in last 1 user year
		orders = Order.objects.filter( order_date__gte = process_start_date, 
			order_date__lte = process_end_date,
			user_id = b.user_id )
		...
		for o in orders:
			...
			bus_Ref = Business_referral_fee (
				...
				business_profile = b,
				order = o,
				fee_amount = o.order_total * perc / 100,
				fee_paid_date = today,
				fee_paid_reference = None,
				created_date = today,
				updated_date = today
			)
