...
def createAmazonFile_kids():
	...
	amz = Amazon_data.objects.filter(is_published = True, product_id__in = ids,
		amazon_sku__gte = 'AV550000',  amazon_sku__lte = 'AV560000').order_by('amazon_sku')
	with open('Amazon_data_advTemplate.csv', 'w', newline='') as myfile:
		...
		for h in amz:
			...
			prod_details = "Wall painting for children / kids room with frame; Product Details:- Framed Art Print; Artist: " + h.artist + "; Title: " + h.product_name + ".\n Printed on: " + h.print_medium_id.title() + "; "
			...
			if h.category_name:
				...
				cat_nm = ''
			if h.image_type == '0':
				prod_name = "Arte' Venue: Beautiful painting for children / kids room with frame, " + h.product_name + '; Artist: ' + h.artist + '; Framed Art Print (' + t_size + ', customizable) ' + cat_nm
			else:
...
def createAmazonFile_vintage():
	
	amz = Amazon_data.objects.filter(is_published = True, amazon_sku__gte = 'AV560000').order_by('amazon_sku')
	with open('Amazon_data_advTemplate.csv', 'w', newline='') as myfile:
		wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)
		row =['product_type', 'Seller SKU', 'Brand', 'Product ID', 'Product ID Type',
			'Item Name', 'Manufacturer Part Number', 'Recommended Browse Nodes', 
		...
		wr.writerow(row)
		for h in amz:
			if h.image_type == '0':	 ##Skip photographs
				continue
			length = formatNumber(h.image_height) 
			if h.moulding:
				if h.moulding.width_inner_inches:
			...
			if (h.category_name).upper() == 'PHOTOGRAPHY':
				prod_details = "Vintage art with frame; Product Details:- Framed Art Print; Artist: " + h.artist + "; Title: " + h.product_name + ".\n Printed on: " + h.print_medium_id.title() + "; "
			else:
				prod_details = "Vintage painting with frame; Product Details:- Framed Art Print; Artist: " + h.artist + "; Title: " + h.product_name + ".\n Printed on: " + h.print_medium_id.title() + "; "
			incl_components = "One piece art with frame printed on " + h.print_medium_id.title() + "; "
			...
			search_terms = ("vintage painting art home decor walls " + h.category_name + " " + h.key_words.replace('|', ' '))
			...
			if h.category_name:
				cat_nm = h.category_name
			else:
				cat_nm = ''
			if h.image_type == '0':
				prod_name = "Arte' Venue: Vintage painting with frame, " + h.product_name + '; Artist: ' + h.artist + '; Framed Art Print (' + t_size + ', customizable) ' + cat_nm
			else:
				prod_name = "Arte' Venue: Vintage Art with frame, " + h.product_name + '; Artist: ' + h.artist + ' - Framed Art Print(' + t_size + ', customizable) '
...
def amz_data_rework():
	...
	amz_data = Amazon_data.objects.all()
	
	with open('Amazon_data_correction.csv', 'w', newline='') as myfile:
		wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)		
		for a in amz_data:
			if a.parent_child == 'P':
				continue
		
			prod = Stock_image.objects.get(pk = a.product_id)
			...
			if a.orientation == 'Horizontal':
				img_width = a.image_height
				img_height = round(img_width / prod.aspect_ratio)
				print(str(img_width) + " - " + str(img_height))
				row = [ a.product_id, a.orientation, a.aspect_ratio, img_width, img_height ]
				wr.writerow(row)


def createAmazonFile_all():
	
	amz = Amazon_data.objects.filter(is_published = True).order_by('amazon_sku')
	with open('Amazon_data_advTemplate.csv', 'w', newline='') as myfile:
		wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)
		row =['product_type', 'Seller SKU', 'Brand', 'Product ID', 'Product ID Type',
			'Item Name', 'Manufacturer Part Number', 'Recommended Browse Nodes', 
...
def createAmazonFile_campaign():
	...
	amz = Amazon_data.objects.filter(is_published = True, product_id__in = ids,
		amazon_sku__gte = 'AVC10001').order_by('amazon_sku')
	with open('Amazon_data_advTemplate.csv', 'w', newline='') as myfile:
		wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)
		row =['product_type', 'Seller SKU', 'Brand', 'Product ID', 'Product ID Type',
			'Item Name', 'Manufacturer Part Number', 'Recommended Browse Nodes', 
		...
		for h in amz:
			...
			if h.image_type == '0':
				prod_name = "Arte' Venue: Floral painting with frame, Title: " + h.product_name + '; Artist: ' + h.artist + '; Framed Art Print (' + t_size + ') '
			else:
				prod_name = "Arte' Venue: Floral art with frame, Title: " + h.product_name + '; Artist: ' + h.artist + ' - Framed Art Print(' + t_size + ') '
