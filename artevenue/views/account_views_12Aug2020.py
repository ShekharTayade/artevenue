...
def my_account(request):
	...
	if request.method == 'POST':
		...
		if request.POST.get('ship_form', 'NONE') != 'NONE':
			if request.POST['ship-shipping_address_id'] != '' and request.POST['ship-shipping_address_id']  is not None:
				id = request.POST['ship-shipping_address_id']
			else :
				id =  0
			try:
				s = User_shipping_address.objects.get( pk = id )
			except User_shipping_address.DoesNotExist:
				s = None
			if s:
				shipping_form = shipping_addressForm( request.POST, prefix = 'ship', instance = s )
			else:
		...
		if request.POST.get('bill_form', 'NONE') != 'NONE':
			if request.POST['bill-billing_address_id'] != '' and request.POST['bill-billing_address_id']  is not None:
				id = request.POST['bill-billing_address_id']
			else :
				id =  0
			try:
				b = User_billing_address.objects.get( pk = id)
			except User_billing_address.DoesNotExist:
				b = None
			if b:
				billing_form = billing_addressForm( request.POST, prefix = 'bill', instance = b )
			else:
