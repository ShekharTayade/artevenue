...
def createAmazonImages():	
	...
	amazon_data = Amazon_data.objects.filter(product_id__in = abstract_ids
		, amazon_sku__startswith = 'AVCA').order_by('amazon_key')  ## Abstract - Campaigns
...
def createAmazonFile_campaign():
	...
	amz = Amazon_data.objects.filter(is_published = True, product_id__in = ids,
		amazon_sku__startswith = 'AVCA').order_by('amazon_sku')
