...
def createAmazonData():
	
	cnt = 0
	sku_suf = "AVA"
	sku_int = 10000  ## for Abstract

	abs = [47642, 122569, 205220, 165362, 247683, 247682, 165197, 259019]
...
def createAmazonImages():	
	...
	##abstract_F_ids = [119513,193908,87845,47869,36120,238888,255004,195569,240176,78373,80171,262895,263713,218461,280822, 170770, 172216, 195362, 174947, 158849, 193120, 195172]
	
	abs = [47642, 122569, 205220, 165362, 247683, 247682, 165197, 259019]

	amazon_data = Amazon_data.objects.filter(product_id__in = abs).order_by('amazon_key')  ## Abstract - Campaigns
	...
	for h in amazon_data:
		...
		if h.moulding:
			if h.mount:
				...
				framed_img = get_amz_FramedImage_api(h.product_id, h.moulding_id, 
					None, float(h.mount_size), image_width)
		...
		#######################################################
		####
		card = get_catalog(request, True, 'feature_1.jpg', 0, 105, 550, 760, h.product_id, m_id, 
			mnt_color, mnt_size, image_width, prod_type, '1')
		low_res_card4 = h.amazon_sku +"_card4_" + h.url[loc:]
		card.save(img_loc + low_res_card4)

		####
		card = get_catalog(request, True, 'feature_2.jpg', 500, 165, 500, 625, h.product_id, m_id, 
			mnt_color, mnt_size, image_width, prod_type, '2')
		low_res_card5 = h.amazon_sku +"_card5_" + h.url[loc:]
		card.save(img_loc + low_res_card5)

		####
		card = get_catalog(request, False, 'feature_3.jpg', 145, 190, 470, 590, h.product_id, m_id, 
			mnt_color, mnt_size, image_width, prod_type, '3')		
		low_res_card3 = h.amazon_sku +"_card3_" + h.url[loc:]
		card.save(img_loc + low_res_card3)

		####
		card = get_catalog(request, False, 'catalog_3.jpg', 195, 165, 515, 315, h.product_id, m_id, 
			mnt_color, mnt_size, image_width, prod_type, '4')
		low_res_card2 = h.amazon_sku +"_card2_" + h.url[loc:]
		card.save(img_loc + low_res_card2)

		####
		card = get_catalog(request, False, 'catalog_4.jpg', 156, 75, 405, 235, h.product_id, m_id, 
			mnt_color, mnt_size, image_width, prod_type, '6')
		...
		hf = Amazon_data.objects.filter(amazon_key = h.amazon_key).update(	
				...
				card1_url = img_url + low_res_card4, card2_url = img_url + low_res_card5,
				card3_url = img_url + low_res_card3, card4_url = img_url + low_res_card2,
				card5_url = img_url + low_res_card1 )
...
def createAmazonFile():
	...
	## Abstract Campaigns
	#ids = [119513,193908,87845,47869,36120,238888,255004,195569,240176,78373,80171,262895,263713,218461,280822, 170770, 172216, 195362, 174947, 158849, 193120, 195172]
	ids = [47642, 122569, 205220, 165362, 247683, 247682, 165197, 259019]
	
	amz = Amazon_data.objects.filter(is_published = True, product_id__in = ids,
		amazon_sku__startswith = 'AVA').order_by('amazon_sku')
		
	with open('Amazon_data_advTemplate_ab.csv', 'w', newline='') as myfile:
