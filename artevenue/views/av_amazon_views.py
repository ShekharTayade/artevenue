...
floral_ids = [171589, 19080, 157695, 158849, 174947, 174472, 193119, 74810, 195184, 172216, 195362, 78301, 264791]
landscape_ids = [220205, 261263, 255411, 220212, 220237, 232260, 255765, 238453, 240438, 255764, 255752, 226887, 42459, 278243, 278241, 244172, 244171, 91325, 42654, 42661, 42657, 42658]
...
def createFLFile():
	av_createAmazonFile("FLORAL", floral_ids)
def createLDFile():
	av_createAmazonFile("LANSDACPE", landsacpe_ids)
...
def av_createAmazonData(cat, ids):
	...
	if cat == "ABSTRACT":
		sku_suf = "AVA"
		sku_int = 15000  ## for Abstract
	if cat == "FLORAL":
		sku_suf = "AVF"
		sku_int = 35000  ## for Abstract
	if cat == "LANDSCAPE":
		sku_suf = "AVL"
		sku_int = 55000  ## for Abstract
...
def av_createAmazonImages(cat, ids):

	amazon_data = Amazon_data.objects.filter(product_id__in = ids,
		amazon_key__gt = 36905).order_by('amazon_key')
...
def av_createAmazonFile(cat, ids):
		
	cat = cat.title()
	amz = Amazon_data.objects.filter(is_published = True, product_id__in = ids,
		amazon_key__gt = 36905).order_by('amazon_sku')
		
	file_nm = 'av_amz_data_' + cat + '.csv'
	...
	with open(file_nm, 'w', newline='') as myfile:
		...
		for h in amz:
			...
			if h.image_type == '0':
				prod_name = "Arte' Venue: " + cat + " painting with frame, Title: " + h.product_name + '; Artist: ' + h.artist + '; Framed Art Print (' + t_size + ') '
			else:
				prod_name = "Arte' Venue: " + cat + " art with frame, Title: " + h.product_name + '; Artist: ' + h.artist + ' - Framed Art Print(' + t_size + ') '
