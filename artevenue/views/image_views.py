...
def get_FramedImage_by_id(request, prod_id, m_id, mount_color='', 
	...
	if  moulding:
		...
		if stretched_canvas == 'YES':
			white_frame = False
			if imgtilt == 'YES':				
				framed_img = tilt_image(framed_img, border_width=border_width, frame_flag=True)
			elif dropshadow == 'YES':
				framed_img = tilt_image(framed_img, border_width=border_width, frame_flag=True)
		else:
			if dropshadow == 'YES':
...
def tilt_image(img, border_width=None, frame_flag=True, frame_depth=None, white_frame=False):
   ...
   #Creating image border
	...
	shadow_width = 100
	shadow = Image.new('L', (shadow_width, height))

	shadow = np.array(shadow)
	shadow = shadow[:, ::-1].copy()

	for i in range(height):
		for j in range(shadow_width):
			if i >= -2*slope*j + height - (-2*slope*shadow_width) - (-slope*border_width):
				shadow[i][j] = 255
			elif i <= -slope*j/20 + height - (-2*slope*shadow_width):
				shadow[i][j] = 255
			else:
				shadow[i][j] = 200

	shadow = cv2.cvtColor(shadow, cv2.COLOR_GRAY2RGB)
	shadow = cv2.GaussianBlur(shadow, (13,13), 15)

	#Merging image and shadow
	image = np.array(image) 
	image = image[:, :, ::-1].copy() 
	image = cv2.cvtColor(image, cv2.COLOR_BGRA2RGB)

	image = cv2.hconcat([shadow, image])
